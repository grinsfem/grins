...
#ifndef GRINS_PHYSICS_FACTORY_INCOMPRESSIBLE_HYPERELASTICITY_H
#define GRINS_PHYSICS_FACTORY_INCOMPRESSIBLE_HYPERELASTICITY_H
...
namespace GRINS
{
  ...
  class PhysicsFactoryIncompressibleHyperelasticity : public PhysicsFactoryWithCore
  {
  public:
    PhysicsFactoryIncompressibleHyperelasticity( const std::string& physics_name,
  ...
  PhysicsFactoryIncompressibleHyperelasticity<Dim,DerivedPhysics>::build_physics
  ( const GetPot & input, const std::string & physics_name )
  {
    ...
    if( strain_energy == std::string("mooney_rivlin") )
      new_physics.reset( new DerivedPhysics<Dim,MooneyRivlin>(physics_name,input) );
    else
      {
...
#endif //GRINS_PHYSICS_FACTORY_INCOMPRESSIBLE_HYPERELASTICITY_H
