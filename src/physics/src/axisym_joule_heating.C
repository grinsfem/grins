...
namespace GRINS
{
  ...
  AxisymmetricJouleHeating::AxisymmetricJouleHeating( const std::string& physics_name,
						      const GetPot& input )
  ...
  AxisymmetricJouleHeating::~AxisymmetricJouleHeating()
  {
    return;
  }

  void AxisymmetricJouleHeating::init_variables( libMesh::FEMSystem* system )
  {
	...
	for (unsigned int i=0; i != n_T_dofs; i++)
	  {
	    ...
	    FT(i) += _sigma*(grad_V*grad_V)*T_phi[i][qp]*r*JxW[qp];

	    if(compute_jacobian)
	      {
		for (unsigned int j=0; j != n_V_dofs; j++)
		  {
		    KTV(i,j) += _sigma*2.0*(grad_V*V_gradphi[j][qp])*T_phi[i][qp]*r*JxW[qp];
		  }
	      }
	  }
      } // end quadrature loop
...
#ifdef USE_GRVY_TIMERS
    this->_timer->EndTimer("AxisymmetricJouleHeating::element_time_derivative");
#endif
