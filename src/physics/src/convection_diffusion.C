...
namespace GRINS
{
  ConvectionDiffusion::ConvectionDiffusion( const GRINS::PhysicsName& physics_name,
    ...
    : Physics(physics_name,input),
      ...
      _kappa("0.0"),
      _var_name(input("Variables/GenericVariables:"+physics_name+"/name", "c") ),
      _FE_family( libMesh::Utility::string_to_enum<GRINSEnums::FEFamily>( input("Variables/GenericVariables:"+physics_name+"/fe_family", "LAGRANGE") ) ),
      _order( libMesh::Utility::string_to_enum<GRINSEnums::Order>(input("Variables/GenericVariables:"+physics_name+"/order", "FIRST")) )
  {
  ...
  {
    _var = system->add_variable( _var_name, _order, _FE_family );
  }

  void ConvectionDiffusion::set_time_evolving_vars( libMesh::FEMSystem* system )
  {
    system->time_evolving(_var);
  }

  void ConvectionDiffusion::init_context( AssemblyContext& context )
  {
    context.get_element_fe(_var)->get_JxW();
    context.get_element_fe(_var)->get_phi();
    context.get_element_fe(_var)->get_dphi();
    context.get_element_fe(_var)->get_xyz();
  }
                                                     ...
                                                     CachedValues& /*cache*/ )
  {
    ...
    libMesh::DenseSubVector<libMesh::Number> &F = context.get_elem_residual(_var);

    libMesh::DenseSubMatrix<libMesh::Number> &K = context.get_elem_jacobian(_var, _var);

    unsigned int n_qpoints = context.get_element_qrule().n_points();

    for (unsigned int qp=0; qp != n_qpoints; qp++)
      {
        libMesh::Number c = context.interior_value(_var, qp);

        libMesh::Gradient grad_c;
	context.interior_gradient(_var, qp, grad_c);
                                           ...
                                           CachedValues& /*cache*/ )
  {
    ...
    libMesh::DenseSubMatrix<libMesh::Number> &M = context.get_elem_jacobian(_var, _var);
    ...
    for (unsigned int qp=0; qp != n_qpoints; qp++)
      {
        libMesh::Real c_dot;
        context.interior_rate(_var, qp, c_dot);
