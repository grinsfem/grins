...
namespace GRINS
{
  ...
  void CachedValues::add_quantity( CachedQuantities quantity )
  {
    _cache_list.insert(quantity);
    return;
  }

  void CachedValues::add_quantities( const std::set<CachedQuantities>& cache_list )
  {
  ...
  bool CachedValues::is_active(CachedQuantities quantity)
  {
    bool value = false;
  ...
  void CachedValues::set_values( CachedQuantities quantity, std::vector<Number>& values )
  {
    _cached_values.insert( std::make_pair( quantity, values ) );
    return;
  }

  void CachedValues::set_vector_values( CachedQuantities quantity, std::vector<std::vector<Number> >& values )
  {
    _cached_vector_values.insert( std::make_pair( quantity, values ) );
    return;
  }

  const std::vector<Number>& CachedValues::get_cached_values( CachedQuantities quantity ) const
  {
    libmesh_assert( _cached_values.find(quantity) != _cached_values.end() );
    return _cached_values.find(quantity)->second;
  }

  const std::vector<std::vector<Number> >& CachedValues::get_cached_vector_values( CachedQuantities quantity ) const
  {
